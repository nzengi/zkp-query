...
/// # Note
/// 
/// Group-By Gate uses Sort Gate output. Group keys must be sorted.
...
pub struct GroupByConfig {
    ...
    // Range Check integration (for additional validation - currently not used)
...
impl GroupByChip {
    ...
    pub fn configure(
        ...
        // 
        // Note: Group-By Gate uses Sort Gate output. Group keys must be sorted.
        ...
        // b = 1 means a new group has started
        // 
        ...
        meta.create_gate("boundary check", |meta| {
            ...
            let inverse_check = p.clone() * diff.clone() - (Expression::Constant(Fr::ONE) - b.clone());
    ...
    pub fn group_and_verify(
        ...
        // Constraints use Rotation::cur() and Rotation::next(), so
            ...
            |mut region| {
                ...
                for i in 0..(group_keys.len() - 1) {
                    ...
                    let (boundary, inverse) = if diff == 0 {
                        ...
                        // But for safety we use unwrap_or(Fr::ZERO)
